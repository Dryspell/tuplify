import { serializeToTuples, toJSON } from "./core.ts";
import {
  isPrimitive,
  Serializable,
  SerializableObject,
  ...
  >(serialized: TypedSerialized<Ttype>): TypedObject<Ttype> {
    ...
    function createProxy(
      ...
      if (!Array.isArray(rep) && repKeys.length !== data.length) {
        throw new Error(
          `Serialized data is missing required fields: ${
            repKeys
              .slice(data.length - 1)
              .join(", ")
          }`,
      ...
      return new Proxy(Array.isArray(rep) ? [] : {}, {
        get(_, prop: string | symbol) {
          if (prop === "toJSON") {
            ...
            return () =>
              Array.isArray(rep)
                ? isPrimitive(data[0]) ? data : data.map(
                  (item, index) => toJSON(rep[0], item),
                )
                : toJSON(rep, data);
          }
          ...
          if (Array.isArray(rep)) {
            ...
            return isPrimitive(dataValue) ? dataValue : createProxy(
        ...
        getPrototypeOf() {
          return Array.isArray(rep) ? Array.prototype : Object.prototype;
        },
        has(_, prop) {
          return Array.isArray(rep)
